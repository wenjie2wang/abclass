...
namespace abclass
{
    ...
    protected:
                                             ...
                                             const double l2_lambda) const
        {
            ...
            for (size_t g{0}; g < control_.penalty_factor_.n_elem; ++g) {
                out += penalty1(beta.row(g + p_data_->inter_),
        ...
        //                                 const double l2_lambda) const
        // {
        //     return iter_loss() * p_data_->div_n_obs_ +
        //         regularization(beta, l1_lambda, l2_lambda);
        // }

        inline arma::vec
        gen_penalty_factor(const arma::vec& penalty_factor = arma::vec()) const
        {
            if (penalty_factor.is_empty()) {
                return arma::ones(p_data_->p0_);
            }
            if (penalty_factor.n_elem == p_data_->p0_) {
            ...
            // else
            throw std::invalid_argument(
                "Incorrect length of the 'penalty_factor'.");
        }
        ...
        inline double mm_gradient0(const unsigned int k) const
        {
            ...
            return arma::accu(grad_k) * p_data_->div_n_obs_;
        }

        // define gradient function at (g, k) for the given inner product
        inline double mm_gradient(const unsigned int g, const unsigned int k)
        {
        ...
        {
            arma::mat out(p_data_->p0_, p_data_->km1_);
            arma::mat grad{iter_dloss_df()};
            for (size_t k{0}; k < p_data_->km1_; ++k) {
                for (size_t j{0}; j < p_data_->p0_; ++j) {
                    out(j, k) = arma::dot(grad.col(k), p_data_->x_.col(j)) *
                        p_data_->div_n_obs_;
                }
            }
            return out;
        ...
        inline virtual void set_lambda_max(const arma::uvec& positive_penalty)
        {
            ...
            for (arma::uvec::const_iterator it{positive_penalty.begin()};
                ...
                if (output_.l1_lambda_max_ < tmp) {
                    output_.l1_lambda_max_ = tmp;
                }
            }
            output_.lambda_max_ = output_.l1_lambda_max_ /
        ...
        // kkt condition
        inline virtual arma::umat
        is_kkt_failed(const arma::umat& is_active_strong,
                      const arma::uvec& positive_penalty,
        ...
        inline virtual void update_beta_gk(arma::mat& beta, const size_t k,
        ...
        inline void run_active_cycles(arma::mat& beta, arma::umat& is_active,
                                      const double l1_lambda,
                                      const double l2_lambda,
    ...
    public:
        ...
        // from Abclass
        using AbclassLinear<T_loss, T_x>::iter_cache_;
        ...
        inline void
        set_penalty_factor(const arma::vec& penalty_factor = arma::vec())
        {
            if (penalty_factor.empty()) {
                control_.penalty_factor_ =
                    gen_penalty_factor(control_.penalty_factor_);
            } else {
    ...
    inline void AbclassCD<T_loss, T_x>::run_one_active_cycle(
        arma::mat& beta, arma::umat& is_active, const double l1_lambda,
        const double l2_lambda, const bool update_active,
        const unsigned int verbose)
    {
        last_eps_ = 0.0;
        if (verbose > 2) {
        ...
        if (p_data_->intercept_) {
            ...
            if (!delta_beta0.is_zero()) {
                ...
                if constexpr (std::is_base_of_v<MarginLoss, T_loss>) {
                    iter_cache_.iter_inner_ +=
                        p_data_->ex_vertex_ * delta_beta0.t();
                } else {
                    iter_cache_.iter_pred_f_.each_row() += delta_beta0;
                }
                last_eps_ = std::max(
                    last_eps_,
        ...
        // for predictors
        for (size_t g{0}; g < p_data_->p0_; ++g) {
            const size_t g1{g + p_data_->inter_};
            for (size_t k{0}; k < p_data_->km1_; ++k) {
                if (is_active(g, k) == 0) {
                    continue;
                }
    ...
    inline void AbclassCD<T_loss, T_x>::run_active_cycles(
        arma::mat& beta, arma::umat& is_active, const double l1_lambda,
        const double l2_lambda, const bool varying_active_set,
        const unsigned int max_iter, const double epsilon,
        const unsigned int verbose)
    {
        converged_ = false;
        size_t num_iter{0};
        ...
        if (varying_active_set) {
            ...
            while (output_.n_iter_ < max_iter) {
                num_iter = 0;
                // cycles over the active set
                while (output_.n_iter_ < max_iter) {
                    Rcpp::checkUserInterrupt();
                    run_one_active_cycle(beta, is_active_varying, l1_lambda,
                                         l2_lambda, true, verbose);
                    ++num_iter;
                    ...
                    if (verbose > 1 || control_.adjust_mm_) {
                        ...
                        double obj1{loss1 * p_data_->div_n_obs_ + pen1};
                        if (verbose > 1) {
                            Rcpp::Rcout << "The objective function changed\n";
                            Rprintf("  from %7.7f (iter_loss: %7.7f + penalty: "
                                    "%7.7f)\n",
                                    last_obj_, last_loss_ * p_data_->div_n_obs_,
                                    last_penalty_);
                            Rprintf("    to %7.7f (iter_loss: %7.7f + penalty: "
                                    "%7.7f)\n",
                                    obj1, loss1 * p_data_->div_n_obs_, pen1);
                        }
                        if (last_obj_ < obj1) {
                            if (verbose > 1) {
                                Rcpp::Rcout
                                    << "Notice: the objective increased.\n";
                            }
                ...
                run_one_active_cycle(beta, is_active, l1_lambda, l2_lambda,
                                     true, verbose);
                ++num_iter;
                ...
                if (arma::accu(arma::any(is_active_varying != is_active))) {
                    ...
                    if (verbose > 0) {
                        Rcpp::Rcout << "Changed the active set from "
                                    << l1_norm(is_active_varying) << " to "
                                    << l1_norm(is_active) << " after "
                                    << num_iter << " iteration(s)\n";
                    }
    ...
    inline void AbclassCD<T_loss, T_x>::fit()
    {
        ...
        arma::uvec positive_penalty{arma::find(control_.penalty_factor_ > 0.0)};
        // initialize
        if constexpr (std::is_base_of_v<MarginLoss, T_loss>) {
            if (p_data_->has_offset_) {
                iter_cache_.iter_inner_ =
                    arma::sum(p_data_->ex_vertex_ % p_data_->offset_, 1);
            } else {
                iter_cache_.iter_inner_ = arma::zeros(p_data_->n_obs_);
            }
        } else {
            if (p_data_->has_offset_) {
                iter_cache_.iter_pred_f_ = p_data_->offset_;
            } else {
                iter_cache_.iter_pred_f_ =
                    arma::zeros(p_data_->n_obs_, p_data_->km1_);
            }
        }
        arma::mat one_beta(p_data_->p1_, p_data_->km1_);
        const bool is_ridge_only{isAlmostEqual(control_.ridge_alpha_, 0.0)};
        double l1_lambda{0.0}, l2_lambda{0.0};
        // if alpha = 0 and customized lambda
        if (is_ridge_only && control_.custom_lambda_) {
            output_.l1_lambda_max_ = -1.0; // not well defined
            output_.lambda_max_ = -1.0;    // not well defined
        } else {
            // need to determine lambda_max
            set_lambda_max(positive_penalty);
            // set up lambda sequence
            if (!control_.custom_lambda_) {
                double log_lambda_max{std::log(output_.lambda_max_)};
                double log_lambda_min{0.0};
                if (control_.lambda_min_ > 0.0) {
                    log_lambda_min = std::log(control_.lambda_min_);
                } else {
                    log_lambda_min =
                        log_lambda_max + std::log(control_.lambda_min_ratio_);
                }
                control_.lambda_ = arma::exp(arma::linspace(
                    log_lambda_max, log_lambda_min, control_.nlambda_));
            }
        }
        // initialize the estimate cube
        output_.coef_ = arma::cube(p_data_->p1_, p_data_->km1_,
                                   control_.lambda_.n_elem, arma::fill::zeros);
        output_.objective_ = output_.penalty_ = output_.loss_ =
            arma::zeros(control_.lambda_.n_elem);
        // get the solution (intercepts) of l1_lambda_max for a warm start
        arma::umat is_active_strong(p_data_->p0_, active_ncol_,
                                    arma::fill::ones);
        // 1) no need to consider possible constant covariates
        is_active_strong.rows(p_data_->x_skip_).zeros();
        ...
        if (p_data_->intercept_) {
            ...
            run_active_cycles(one_beta, is_active_strong,
                              0, // does not matter
                              0, // does not matter
                              false, epsilon0, control_.epsilon_,
                              control_.verbose_);
            last_loss_ = null_loss_ = iter_loss();
        ...
        if (is_ridge_only) {
            for (size_t li{0}; li < control_.lambda_.n_elem; ++li) {
                ...
                run_active_cycles(one_beta, is_active_strong2, l1_lambda,
                                  l2_lambda, false, control_.max_iter_,
                                  epsilon0, control_.verbose_);
                output_.coef_.slice(li) = rescale_coef(one_beta);
                output_.loss_(li) = iter_loss();
                output_.penalty_(li) =
                    regularization(one_beta, l1_lambda, l2_lambda);
                output_.objective_(li) =
                    output_.loss_(li) * p_data_->div_n_obs_ +
                    output_.penalty_(li);
                // if max iter is reached
                if (!converged_) {
                    size_t li1{li + 1};
                    output_.coef_ = output_.coef_.head_slices(li1);
                    control_.lambda_ = control_.lambda_.head(li1);
                    output_.loss_ = output_.loss_.head(li1);
                    output_.penalty_ = output_.penalty_.head(li1);
                    output_.objective_ = output_.objective_.head(li1);
                    break;
                }
            }
            iter_cache_.reset_cache();
            return; // early exit
        }
        ...
        positive_penalty =
            positive_penalty.elem(arma::find(mm_lowerbound_ > 0.0));
        // for strong rule
        double old_l1_lambda{output_.l1_lambda_max_};
        ...
        for (size_t li{0}; li < control_.lambda_.n_elem; ++li) {
            ...
            if (l1_lambda >= output_.l1_lambda_max_) {
                output_.coef_.slice(li) = rescale_coef(one_beta);
                output_.loss_(li) = null_loss_;
                output_.penalty_(li) =
                    regularization(one_beta, l1_lambda, l2_lambda);
                output_.objective_(li) =
                    output_.loss_(li) * p_data_->div_n_obs_ +
            ...
            // update active set by strong rule
            apply_strong_rule(is_active_strong, l1_lambda, old_l1_lambda,
                              positive_penalty);
            ...
            while (kkt_failed) {
                ...
                // update beta
                run_active_cycles(one_beta, is_active_strong, l1_lambda,
                                  l2_lambda, control_.varying_active_set_,
                                  control_.max_iter_, epsilon0,
                                  control_.verbose_);
                if (control_.verbose_ > 0) {
                    msg("Checking the KKT condition for the null set.");
                }
                // check kkt condition
                arma::umat is_strong_rule_failed{is_kkt_failed(
                    is_active_strong_old, positive_penalty, l1_lambda)};
                if (arma::accu(is_strong_rule_failed) > 0) {
                    is_active_strong =
                        is_active_strong_old || is_strong_rule_failed;
                    if (control_.verbose_ > 0) {
            ...
            if (p_data_->et_npermuted_ > 0) {
                ...
                if (!permuted_beta.is_zero()) {
                    if (li == 0) {
                        ...
                        output_.coef_ = output_.coef_.head_slices(1);
                        output_.loss_ = output_.loss_(0);
                        output_.penalty_ = output_.penalty_(0);
                        output_.objective_(0) =
                            output_.loss_(0) * p_data_->div_n_obs_ +
                            output_.penalty_(0);
                    } else {
                        // discard the estimates from this itaration
                        output_.coef_ = output_.coef_.head_slices(li);
                        output_.loss_ = output_.loss_.head(li);
                        output_.penalty_ = output_.penalty_.head(li);
                        output_.objective_ = output_.objective_.head(li);
                    }
                    output_.et_l1_lambda0_ = old_l1_lambda;
                    output_.et_l1_lambda1_ = l1_lambda;
                    if (control_.verbose_ > 0) {
                ...
                if (li == control_.lambda_.n_elem - 1) {
                    ...
                    output_.et_l1_lambda0_ = l1_lambda;
                    output_.et_l1_lambda1_ = -1.0; // tell fit() to ignore
                }
            }
            output_.coef_.slice(li) = rescale_coef(one_beta);
            output_.loss_(li) = iter_loss();
            output_.penalty_(li) =
                regularization(one_beta, l1_lambda, l2_lambda);
            output_.objective_(li) =
                output_.loss_(li) * p_data_->div_n_obs_ + output_.penalty_(li);
            // if max iter is reached
            if (!converged_) {
                size_t li1{li + 1};
                output_.coef_ = output_.coef_.head_slices(li1);
