...
namespace abclass
{
    ...
    protected:
                                   ...
                                   const double l2_lambda) override
        {
            ...
            const double l1_lambda_g0{l1_lambda * control_.penalty_factor_(g)};
            // local approximation
            const MellowmaxL1 mlm{beta.row(g1), control_.mellowmax_omega_};
            const double inner_pen{mlm.value()};
            const double outer_dpen{
                dmcp_penalty(inner_pen, l1_lambda_g0, control_.ncv_gamma_)};
            const arma::rowvec dvec{mlm.grad()};
            const double mlm_dk{dvec(k)};
            const double mlm_d2k{control_.mellowmax_omega_ * mlm_dk *
                                 (1 - mlm_dk)};
            const double outer_dpen2{outer_dpen * mlm_d2k};
            const double u_g{(mm_lowerbound_(g) + outer_dpen2) * old_beta_g1k -
                             d_gk};
            const double l1_lambda_g{outer_dpen * mlm_dk};
            const double tmp{std::abs(u_g) - l1_lambda_g};
            if (tmp > 0.0) {
                const double numer{tmp * sign(u_g)};
                const double denom{mm_lowerbound_(g) + l2_lambda + outer_dpen2};
                // update beta
                beta(g1, k) = std::max(
            ...
            if (delta_beta != 0.0) {
                ...
                last_eps_ =
                    std::max(last_eps_, mlm_d2k * delta_beta * delta_beta);
            }
        }
