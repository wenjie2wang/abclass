    ...
    )
{
    ...
    control.set_intercept(intercept)->
        ...
        tune_et(nstages);
    return lum_gscad<arma::mat>(x, y, control, main_fit, lum_a, lum_c);
}

// [[Rcpp::export]]
Rcpp::List r_lum_gscad_sp(
    const arma::sp_mat& x,
    const arma::uvec& y,
    const arma::vec& lambda,
    const double alpha,
    ...
    )
{
    ...
    control.set_intercept(intercept)->
        ...
        tune_cv(nfolds, stratified, alignment)->
        tune_et(nstages);
    return lum_gscad<arma::sp_mat>(x, y, control, main_fit, lum_a, lum_c);
}
